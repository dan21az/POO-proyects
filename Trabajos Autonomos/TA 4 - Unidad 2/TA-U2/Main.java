...
class Main {
  public static void main(String[] args) {
    ...
    for(int i =0;i<=4;i++){
      ...
      Servidor s = new Servidor(nombre, ip, cantidadProcesador, disco, memoria); //Crear Objeto
      listaServers[i] = s;
      entrada.nextLine();
    }
    //Consultar servidor
    ...
    for (Servidor server: listaServers){
      if (server.getIp().equals(ipConsulta)){
        server.mostrarInformacion();
        resultado = true;
      }
      if (resultado == false){
